...
var require_supabase = __commonJS({
  "../../packages/database/dist/supabase.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.supabase = void 0;
    var supabase_js_1 = require_dist4();
    var supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL || "https://qjhrlqpsfzaycoaekwqh.supabase.co";
    var supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || "sb_publishable_c0x3S20RA_xIVMed3cPpFQ_kO1XXul6";
    exports2.supabase = (0, supabase_js_1.createClient)(supabaseUrl, supabaseKey);
  }
});
...
var require_dist5 = __commonJS({
  "../../packages/database/dist/index.js"(exports2) {
    ...
    if (process.env.NODE_ENV !== "production") {
      globalThis.prisma = exports2.prisma;
    }
    __exportStar2(require("@prisma/client"), exports2);
    __exportStar2(require_supabase(), exports2);
  }
});
...
// src/routes.ts
var import_database = __toESM(require_dist5());
var import_database2 = __toESM(require_dist5());
var activeSettings = {
  ...
  activeClientConfig: {
    name: "Smash & Co.",
    tagline: "Hamb\xFArgueres Artesanais Grelhados na Brasa",
    logoIcon: "\u{1F354}",
    logoUrl: "",
    primaryColor: "#FF5A1F",
  ...
  playlists: {
    "tv-caixa": [
      ...
      { id: "c-2", name: "Combo Ter\xE7a Double Smash", type: "image", duration: 8, transition: "fade", showPrice: true, showIngredients: false, showQr: true, themeColor: "#1C120C", fontFamily: "Outfit", mediaUrl: "/foto.png" }
    ],
    "tv-salao": [
      ...
      { id: "s-4", name: "Combo Ter\xE7a Double Smash", type: "image", duration: 8, transition: "fade", showPrice: true, showIngredients: false, showQr: true, themeColor: "#1C120C", fontFamily: "Outfit", mediaUrl: "/foto.png" }
    ],
    "tv-delivery": [
    ...
    "tv-outdoor": [
      { id: "o-1", name: "Combo Ter\xE7a Double Smash", type: "image", duration: 8, transition: "fade", showPrice: true, showIngredients: false, showQr: true, themeColor: "#1C120C", fontFamily: "Outfit", mediaUrl: "/foto.png" }
    ]
  },
  turnos: {
...
async function registerRoutes(fastify2) {
  ...
  fastify2.get("/api/settings", async () => {
    try {
      const { data } = await import_database2.supabase.from("tv_settings").select("*").eq("id", "default").single();
  ...
  fastify2.post("/api/settings", async (request) => {
    const data = request.body;
    try {
      const { data: dbSettings } = await import_database2.supabase.from("tv_settings").select("*").eq("id", "default").single();
      if (dbSettings) {
        activeSettings = {
          ...activeSettings,
    ...
    try {
      await import_database2.supabase.from("tv_settings").upsert({
        id: "default",
  ...
  fastify2.get("/health", async (request, reply) => {
    try {
      ...
      return { status: "ok", database: "connected", timestamp: /* @__PURE__ */ new Date() };
    } catch (error) {
  ...
  fastify2.get("/api/restaurants", async () => {
    return import_database.prisma.restaurant.findMany({
      orderBy: { createdAt: "desc" }
    });
  });
  fastify2.post("/api/restaurants", async (request, reply) => {
    const { name, slug, logoUrl, primaryColor, secondaryColor } = request.body;
    if (!name || !slug) {
      return reply.status(400).send({ error: "Name and slug are required" });
    }
    try {
      const restaurant = await import_database.prisma.restaurant.create({
  ...
  fastify2.get("/api/campaigns", async () => {
    return import_database.prisma.campaign.findMany({
      include: { restaurant: true, assets: true, tasks: true },
      orderBy: { createdAt: "desc" }
    });
  });
  fastify2.post("/api/campaigns", async (request, reply) => {
    const { restaurantId, name, theme } = request.body;
    if (!restaurantId || !name || !theme) {
      return reply.status(400).send({ error: "restaurantId, name, and theme are required" });
    }
    try {
      const campaign = await import_database.prisma.campaign.create({
        data: {
          restaurantId,
          name,
          theme,
      ...
      await import_database.prisma.agentTask.create({
        data: {
  ...
  fastify2.get("/api/products", async () => {
    try {
      const products = await import_database.prisma.product.findMany({
  ...
  fastify2.post("/api/products", async (request, reply) => {
    const data = request.body;
    try {
      const product = await import_database.prisma.product.create({
        data: {
          nome: data.nome,
  ...
  fastify2.put("/api/products/:id", async (request, reply) => {
    const { id } = request.params;
    const data = request.body;
    try {
      const product = await import_database.prisma.product.update({
        where: { id },
        data: {
          nome: data.nome,
  ...
  fastify2.delete("/api/products/:id", async (request, reply) => {
    const { id } = request.params;
    try {
      await import_database.prisma.product.delete({ where: { id } });
      return { success: true };
    } catch (error) {
      return reply.status(400).send({ error: error.message });
    }
  });
  fastify2.get("/api/promotions", async () => {
    try {
      const promotions = await import_database.prisma.promotion.findMany({
  ...
  fastify2.get("/api/tv/sync", async () => {
    return {
      status: "active",
...
var server = (0, import_fastify.default)({
  logger: true,
  bodyLimit: 104857600
  // 100MB limit for rich media, playlists, and base64 assets
});
